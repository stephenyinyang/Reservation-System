...
function getGoals(req, res, next) {
    userService.getGoals(req.params.username).then((data) => res.json(data)).catch(err => next(err));
}
...
function deleteUser(req, res, next) {
  userService.deleteUser(req.params.username).then(() => res.json("Deleted: 1")).catch(err => next(err));
}
// req.user.username
